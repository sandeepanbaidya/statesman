...
public class HttpFilePipedAction extends BaseAction<HttpFilePipedActionTemplate> {
    ...
    protected JsonNode execute(HttpFilePipedActionTemplate httpFilePipedActionTemplate, Workflow workflow) {
        ...
        String responseTranslator = httpFilePipedActionTemplate.getResponseTranslator();
        HttpActionData httpActionData = transformSourcePayload(workflow, httpFilePipedActionTemplate);
        ...
        byte[] file = (byte[]) fetchFile(httpActionData);

        httpActionData = transformTargetPayload(workflow, httpFilePipedActionTemplate, file);
    ...
    private Response executeRequest(HttpActionData actionData) {
        return actionData.getMethod().visit(new HttpMethod.MethodTypeVisitor<Response>() {
            ...
            private final Map<String, String> fileMeta = actionData.getFileMeta();
            ...
            public Response visitForm() throws Exception {
                ...
                Response response = client.get().form(url, file, payload, fileMeta, headers);
                if (!isSuccessful(response, actionData)) {
